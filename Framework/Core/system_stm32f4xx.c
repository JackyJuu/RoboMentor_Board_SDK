 ...
 * Hardware configuration needed for Nucleo Board: – SB54, SB55 OFF – R35
 * removed – SB16, SB50 ON */
...
#endif                          /* USE_HSE_BYPASS */
#else
#endif                          /* STM32F40_41xxx || STM32F427_437xx ||
                                 * STM32F429_439xx || STM32F401xx ||
                                 * STM32F469_479xx */
...
void SystemCoreClockUpdate(void)
{
  ...
  case 0x08:                   /* PLL P used as system clock source */
    ...
    if (pllsource != 0)
    {
      /* HSE used as PLL clock source */
      pllvco =
        (HSE_BYPASS_INPUT_FREQUENCY / pllm) *
...
/******************************************************************************/
  ...
  {                             /* If HSE fails to start-up, the application
                                 * will have wrong clock configuration. User
                                 * can add here some code to deal with this
                                 * error */
  }
#elif defined(STM32F410xx) || defined(STM32F411xE)
#if defined(USE_HSE_BYPASS)
...
/*-- GPIOs Configuration -----------------------------------------------------*/
   ...
   * PD1 <-> FMC_D3 | PE1 <-> FMC_NBL1 | PF1 <-> FMC_A1 | PG1 <-> FMC_A11 | | PD4 
   * <-> FMC_NOE | PE3 <-> FMC_A19 | PF2 <-> FMC_A2 | PG2 <-> FMC_A12 | | PD5 <-> 
   * FMC_NWE | PE4 <-> FMC_A20 | PF3 <-> FMC_A3 | PG3 <-> FMC_A13 | | PD8 <->
   * FMC_D13 | PE7 <-> FMC_D4 | PF4 <-> FMC_A4 | PG4 <-> FMC_A14 | | PD9 <->
   * FMC_D14 | PE8 <-> FMC_D5 | PF5 <-> FMC_A5 | PG5 <-> FMC_A15 | | PD10 <->
   * FMC_D15 | PE9 <-> FMC_D6 | PF12 <-> FMC_A6 | PG9 <-> FMC_NE2 | | PD11 <->
   * FMC_A16 | PE10 <-> FMC_D7 | PF13 <-> FMC_A7 |-----------------+ | PD12 <->
   * FMC_A17 | PE11 <-> FMC_D8 | PF14 <-> FMC_A8 | | PD13 <-> FMC_A18 | PE12 <->
   * FMC_D9 | PF15 <-> FMC_A9 | | PD14 <-> FMC_D0 | PE13 <-> FMC_D10
   * |-----------------+ | PD15 <-> FMC_D1 | PE14 <-> FMC_D11 | | | PE15 <->
   * FMC_D12 | +------------------+------------------+ */
...
/*-- FMC Configuration ------------------------------------------------------*/
  ...
  /* 
   * Bank1_SRAM2 is configured as follow: In case of FSMC configuration
   ...
   * 
   * In case of FMC configuration NORSRAMTimingStructure.FMC_AddressSetupTime
   * = 1; NORSRAMTimingStructure.FMC_AddressHoldTime = 0;
