...
int InternalFlash_Test(void)
{
  ...
  while (uwSectorCounter <= uwEndSector) 
  {
	...
	USART1_printf("所选扇区擦除完毕\r\n");
  /* 以“字”的大小为单位写入数据 ********************************/
  uwAddress = FLASH_USER_START_ADDR;
	USART1_printf("开始从扇区写数据\r\n");
  while (uwAddress < FLASH_USER_END_ADDR)
  {
    ...
    { 
      /*写入出错，返回，实际应用中可加入处理 */
			USART1_printf("数据写入失败\r\n");
			return -1;
			
    }
  }
	USART1_printf("数据写入成功\r\n");
  ...
  {    
		USART1_printf("数据读出错误\r\n");
		return -1;
		
  }
  else /*数据校验正确*/
  { 
		USART1_printf("数据读出成功\r\n");
		return 0;   		
  }
}
...
*/
int Flash_Wordbyte_Write(uint32_t Start_Address,uint8_t *Receive_DATA,int DATA_num)
{
...
*/
uint32_t Flash_Wordbyte_Read(uint32_t Start_Address,uint8_t *Receive_DATA,int DATA_num)
{
	uint32_t uwAddress = 0;							//开始读取开始地址
	uint32_t End_Address = 0;
	int num = 0;
...
*/
int Flash_HalfWordbyte_Write(uint32_t Start_Address,uint8_t *Receive_DATA,int DATA_num)
{
	uint32_t uwAddress = 0;							//开始读取开始地址
	uint32_t End_Address = 0;
	uint16_t DATA = 0;
...
读以半字为单位的数据 即两个字节
*/
uint16_t Flash_HalfWordbyte_Read(uint32_t Start_Address,int DATA_num,uint16_t *Receive_DATA)
{
	uint32_t uwAddress = 0;							//开始读取开始地址
	uint32_t End_Address = 0;
	int num = 0;
...
读一个字节
*/
uint8_t Flash_Byte_Read(uint32_t Start_Address,int DATA_num,uint8_t *Receive_DATA)
{
	uint32_t uwAddress = 0;							//开始读取开始地址
	uint32_t End_Address = 0;
	int num = 0;
